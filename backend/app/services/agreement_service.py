    ...
    *,
    ...
    if raw_text:
        try:
            ...
            agreement.extraction_at = datetime.now(tz=UTC)
...
async def list_agreements(session: AsyncSession) -> list[Agreement]:
    result = await session.execute(select(Agreement).order_by(Agreement.name))
    return list(result.scalars().all())


async def get_agreement(session: AsyncSession, agreement_id: uuid.UUID) -> Agreement:
    result = await session.execute(
        select(Agreement).where(Agreement.id == agreement_id).options(selectinload(Agreement.results))
    )
...
async def check_invoice(
    ...
    ag_result = await session.execute(select(Agreement).where(Agreement.id == agreement_id))
